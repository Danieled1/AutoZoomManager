...
router.get("/:userId", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/users/${userId}?${qs.stringify({ status })}`,
      headerConfig
    );
    return res.json(request.data);
  } catch (err) {
...
router.get("/:userId/settings", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/users/${userId}/settings`,
      headerConfig
    );
    return res.json(request.data);
  } catch (err) {
...
router.patch("/:userId", async (req, res) => {
  ...
  try {
    const request = await axios.patch(
      `${ZOOM_API_BASE_URL}/users/${userId}`,
      body,
      headerConfig
    );
    return res.json(request.data);
...
router.delete("/:userId", async (req, res) => {
  ...
  try {
    const request = await axios.delete(
      `${ZOOM_API_BASE_URL}/users/${userId}?${qs.stringify({ action })}`,
      headerConfig
    );
    return res.json(request.data);
  } catch (err) {
...
router.get("/:userId/meetings", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/users/${userId}/meetings?${qs.stringify({
        from,
        to,
        next_page_token,
      })}`,
...
router.get("/:userId/webinars", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/users/${userId}/webinars?${qs.stringify({
        next_page_token,
      })}`,
      headerConfig
    );
    ...
    return errorHandler(
      err,
      res,
      `Error fetching webinars for user: ${userId}`
    );
  }
});
...
router.get("/:userId/recordings", async (req, res) => {
  ...
  try {
    const request = await axios.get(
      `${ZOOM_API_BASE_URL}/users/${userId}/recordings?${qs.stringify({
        from,
        to,
        next_page_token,
      })}`,
    ...
    return errorHandler(
      err,
      res,
      `Error fetching recordings for user: ${userId}`
    );
  }
});
router.post("/proxy-download", async (req, res) => {
  ...
  //   res.setHeader("Content-Disposition", `attachment; filename="${topic}.mp4"`);
  //   res.setHeader("Content-Type", "video/mp4");

  //   res.send(response.data);
  res.send("test");
  // } catch (error) {
  //   console.error("Error sending recording file:", error);
  //   res.status(500).send("Error sending recording file");
  // }
});
